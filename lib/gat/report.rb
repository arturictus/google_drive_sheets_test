...
module Gat
  class Report # rubocop:disable Style/Documentation, Metrics/ClassLength
    REPORT_SHEET = "System export"
    def initialize
      @config = Config.new
    end
    ...
    def update(report) # rubocop:disable Metrics/MethodLength
      mark_as_updating
      # remove sheet[1] if exists
      create_export_sheet(report)
      range = full_range(report_sheet)
    ...
    def create_export_sheet(report) # rubocop:disable Metrics/AbcSize
      ...
      puts ">>>>>>>>>> Create sheet: #{response.inspect}"
    end
    ...
    def report_sheet
      spreadsheet.sheets.find { |s| s.properties.title == REPORT_SHEET }
    end
